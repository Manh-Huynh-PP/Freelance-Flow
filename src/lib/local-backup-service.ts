...
export class LocalBackupService {
  ...
  /**
   * Liệt kê các file backup trong thư mục (JSON only; Excel disabled)
   */
  static async listBackupFiles(): Promise<{ name: string; lastModified: number; size: number; type: 'json' }[]> {
    if (!this.directoryHandle) return [];
    const files: { name: string; lastModified: number; size: number; type: 'json' }[] = [];
    try {
      ...
      for await (const [name, handle] of (this.directoryHandle as any).entries()) {
        ...
        try {
          ...
          files.push({ name, lastModified: file.lastModified, size: file.size, type: 'json' });
        } catch {}
      }
    } catch (e) {
