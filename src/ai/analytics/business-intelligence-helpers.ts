  ...
  const calculateFixedCosts = () => {
    ...
    const result = appData.fixedCosts.reduce((total, cost) => {
      ...
      if (!cost.startDate || typeof cost.amount !== 'number' || isNaN(cost.amount)) {
        console.warn(`⚠️ Skipping fixed cost "${cost.name}" due to invalid data (startDate: ${cost.startDate}, amount: ${cost.amount})`);
        return total;
      }
      ...
      if (isNaN(costStart.getTime())) {
        console.warn(`⚠️ Skipping fixed cost "${cost.name}" due to invalid startDate: ${cost.startDate}`);
        return total;
      }
    ...
    if (Array.isArray(payments) && payments.length > 0) {
      payments.forEach(p => {
        if (p && p.status === 'paid') {
          ...
          const rawPaymentDate = p.date;
          const quotePaidDate = (q as any).paidDate;
          const selectedDate = rawPaymentDate || quotePaidDate || t.deadline || t.endDate || t.startDate;
          const paymentDate = toDate(selectedDate);

          // DEBUG: Log which date is being used
          console.log(`📅 [Monthly] Task "${t.name}" payment:`, {
            rawPaymentDate,
            quotePaidDate,
          ...
          if (paymentDate && (useAllTime || (normFrom && normTo && paymentDate >= normFrom && paymentDate <= normTo))) {
            ...
            console.log(`   → Added to month ${monthKey}: ${amount}`);
          }
        }
      });
    } else if (typeof (q as any).amountPaid === 'number') {
