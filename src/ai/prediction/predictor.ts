...
export class CorePredictionEngine {
  ...
  async generatePredictions(context: PredictionContext): Promise<TaskPrediction[]> {
    ...
    try {
      ...
      console.log(`🔮 Generated ${finalPredictions.length} predictions with avg confidence: ${finalPredictions.reduce((sum, p) => sum + p.confidence, 0) / finalPredictions.length
        }`);
