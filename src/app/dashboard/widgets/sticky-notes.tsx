...
import type { AppSettings, Note, AppData } from '@/lib/types';
...
export function StickyNotes({ settings }: StickyNotesProps) {
    ...
    // Use notes from appData, fallback to empty array
    ...
    // Removed localStorage logic as we now use appData.notes directly
    ...
    const handleDeleteNote = (id: number) => {
        setAppData((prev: AppData) => ({ ...prev, notes: (prev.notes || []).filter((note: Note) => note.id !== id) }));
    };
    ...
    const handleSaveNote = () => {
        ...
        if (activeNote) {
            setAppData((prev: AppData) => ({
                ...prev,
                notes: (prev.notes || []).map((n: Note) => n.id === activeNote.id ? { ...n, text: currentHTML } : n)
            }));
        } else {
            setAppData((prev: AppData) => ({
                ...prev,
                notes: [...(prev.notes || []), { id: Date.now(), text: currentHTML }]
            }));
        }
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            setAppData((prev: AppData) => ({ ...prev, notes: arrayMove(prev.notes || [], oldIndex, newIndex) }));
        }
        setIsDragging(false);
        setActiveId(null);
    };
    ...
    const activeNoteForOverlay = activeId ? notes.find((n: Note) => n.id === activeId) : null;
