...
import { useAppData } from '@/hooks/useAppData-supabase';
...
function DashboardDataProvider({ children }: { children: ReactNode }) {
    ...
    // Sync work sessions to Supabase when they change
    const workTime = useWorkTimeData(
        data.appData?.workSessions,
        (newSessions) => {
            // Only sync if data has actually loaded to avoid overwriting with empty
            if (data.isDataLoaded) {
                data.saveAppData({ workSessions: newSessions });
            }
        }
    );
    ...
    // Soft Delete project (and update tasks to remove projectId)
    ...
    const permanentDeleteProject = useCallback((projectId: string) => {
        setAppData(prev => {
            ...
            // Remove projectId from tasks that were in this project
    ...
    const handleRestoreProject = useCallback((id: string) => { restoreProject(id); }, [restoreProject]);
    const handlePermanentDeleteProject = useCallback((id: string) => { permanentDeleteProject(id); }, [permanentDeleteProject]);
    ...
    const handleEmptyTrash = useCallback(() => {
        setAppData((prev: AppData) => ({
            ...
            projects: (prev.projects || []).filter(p => !p.deletedAt)
        }));
    }, [setAppData]);
    ...
    const handleClearOnlyData = useCallback(() => {
        ...
        (async () => {
            // Clear persisted docs and legacy localStorage AI blocks so cards show cleared state
            try {
            ...
            // Clear remote data (DB and Shared Links)
            try {
                await SupabaseDataService.clearAllUserData();
                // Auth fetch to clear shares via API
                await authFetch('/api/share/clear-all', { method: 'DELETE' });
            } catch (e) {
                console.warn('Failed to clear remote user data', e);
            }
        })();
    }, [setAppData]);

    // Clear main data and backups together
    const handleClearDataAndBackups = useCallback(async () => {
        try {
            // Clear remote data first (DB and Shared Links)
            await SupabaseDataService.clearAllUserData();
            await authFetch('/api/share/clear-all', { method: 'DELETE' });
        } catch (e) {
            console.warn('Failed to clear remote user data', e);
            ...
            toast({
                ...
                description: "Could not clear remote data. Please try again or check your connection.",
                variant: "destructive",
                duration: 5000,
            });
            return; // Don't proceed if remote clear fails
        }

        // Clear local data only after remote clear succeeds
        setAppData(() => ({ ...emptyAppData } as AppData));
        ...
        try {
            ...
            console.warn('Failed to clear localStorage keys during clear-data-and-backups', err);
        }
    }, [setAppData, toast]);
