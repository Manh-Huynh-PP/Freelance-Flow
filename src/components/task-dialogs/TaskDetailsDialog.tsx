...
// Lightweight Share modal state - now with Tabs for Share Link and Export PDF
function ShareConfirmModal({
  open,
  onOpenChange,
  defaultIncludeQuote = true,
  defaultIncludeTimeline = true,
  defaultHideTimelineColumn = false,
  defaultShowValidityNote = true,
  defaultShowBriefLinks = true,
  defaultShowDriveLinks = true,
  onConfirm,
  onExportPdf,
  t,
  isLoading,
  ...
  defaultIncludeQuote?: boolean;
  defaultIncludeTimeline?: boolean;
  defaultHideTimelineColumn?: boolean;
  defaultShowValidityNote?: boolean;
  defaultShowBriefLinks?: boolean;
  defaultShowDriveLinks?: boolean;
  onConfirm: (opts: {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {t.shareSelectWhat || "Select what to include"}
          </DialogDescription>
        </DialogHeader>
        ...
        <Tabs defaultValue="link" className="w-full">
          ...
          <TabsContent value="link" className="space-y-3 pt-2">
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                disabled={isLoading}
              />
              {t.includeTimeline || "Include Timeline"}
            </label>
            <label className="flex items-center gap-2 opacity-100">
              <input
                type="checkbox"
                checked={hideTimelineColumn}
                onChange={(e) => setHideTimelineColumn(e.target.checked)}
                disabled={isLoading || !includeQuote}
              />
            ...
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={showValidityNote}
                onChange={(e) => setShowValidityNote(e.target.checked)}
                disabled={isLoading || !includeQuote}
              />
            ...
            <div className="border-t pt-2 mt-2">
              <div className="text-xs font-medium text-muted-foreground mb-2">
                {t.linksDisplay || "Links Display"}
              </div>
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={showBriefLinks}
                  onChange={(e) => setShowBriefLinks(e.target.checked)}
                  disabled={isLoading}
                />
                {t.showBriefLinks || "Show Brief Links"}
              </label>
              <label className="flex items-center gap-2 mt-2">
                <input
                  type="checkbox"
                  checked={showDriveLinks}
                  onChange={(e) => setShowDriveLinks(e.target.checked)}
                  disabled={isLoading}
                />
                {t.showDriveLinks || "Show Drive Links"}
              </label>
            </div>
            {isLoading && (
          ...
          <TabsContent value="pdf" className="space-y-3 pt-2">
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={pdfIncludeQuote}
                onChange={(e) => setPdfIncludeQuote(e.target.checked)}
            ...
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={pdfIncludeTimeline}
                onChange={(e) => setPdfIncludeTimeline(e.target.checked)}
                disabled={isExportingPdf}
              />
              {t.includeTimeline || "Include Timeline"}
            </label>
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={pdfShowValidityNote}
                onChange={(e) => setPdfShowValidityNote(e.target.checked)}
            ...
            <p className="text-xs text-muted-foreground">
              {t.pdfDescription || "Download a PDF with Quote and Timeline."}
            </p>
            <DialogFooter>
              <Button
                variant="secondary"
              ...
              <Button
                ...
                disabled={
                  (!pdfIncludeQuote && !pdfIncludeTimeline) || isExportingPdf
                }
              >
                <FileDown className="w-4 h-4 mr-2" />
                {isExportingPdf
                  ? t.generatingPdf || "Generating..."
...
export function TaskDetailsDialog({
  ...
  // Initialize toast and translations early for use in callbacks
  const { toast } = useToast();
  ...
  const stableUpdateQuoteHandler = useCallback((quoteId: string, updates: Partial<Quote>) => {
    ...
    if (isCollaboratorQuote) {
      ...
      if (handler) {
        ...
        // Show success toast for collaborator quote updates
        toast({
          title: T.saveSuccess || "Đã lưu thành công!",
          description: T.collaboratorQuoteUpdated || "Báo giá collaborator đã được cập nhật.",
        });
      } else {
    ...
    if (handler) {
      ...
      // Show success toast for quote updates
      // Determine what was updated to show appropriate message
      let description = T.quoteUpdated || "Báo giá đã được cập nhật.";
      if ('paymentProgress' in updates || 'payments' in updates) {
        description = T.paymentUpdated || "Thông tin thanh toán đã được cập nhật.";
      }
      toast({
        title: T.saveSuccess || "Đã lưu thành công!",
        description,
  ...
  }, [onUpdateQuote, contextUpdateQuote, onUpdateCollaboratorQuote, contextUpdateCollaboratorQuote, collaboratorQuotes, toast, T]);
  ...
  const onExportPdf = async ({ includeQuote, includeTimeline, showValidityNote }: { includeQuote: boolean; includeTimeline: boolean; showValidityNote: boolean }) => {
    ...
    try {
      ...
      const safeTaskName = (task.name || 'project-report')
      ...
      toast({ variant: 'destructive', title: T.exportFailed || 'Export failed', description: e?.message || String(e) });
    } finally {
  ...
  // State for editing collaborator quotes
          ...
          {quoteData && (
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                size="sm"
                className="h-7 px-2"
                onClick={onEdit || (() => setIsPriceEditOpen(true))}
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[80vw] max-w-5xl h-[80vh] min-h-[600px] max-h-[90vh] flex flex-col">
        ...
        <span className="sr-only">
          ...
          <DialogDescription>Task Details View</DialogDescription>
        </span>
        {/* --- HEADER: Project Info --- */}
                  ...
                  {taskCollaboratorQuotes.map((collabQuote, index) => {
                    ...
                    return (
                      <div key={`${collabQuote.id}-${index}`} className="space-y-2">
                        ...
                        {renderQuoteTable(
                          `${T.collaboratorCosts} - ${collaborator?.name || `Collaborator ${index + 1}`}`,
                          collabQuote,
                          () => {
                            setEditingCollabQuoteId(collabQuote.id);
                            setIsCollabEditOpen(true);
                          }
                        )}
                        {/* Edit Collaborator Quote Dialog */}
                        {editingCollabQuoteId === collabQuote.id && (
                          <TimelineEditDialog
                            isOpen={isCollabEditOpen}
                            onClose={() => {
                              setIsCollabEditOpen(false);
                              setEditingCollabQuoteId(null);
                            }}
                            task={task}
                            quote={collabQuote}
                            settings={settings}
