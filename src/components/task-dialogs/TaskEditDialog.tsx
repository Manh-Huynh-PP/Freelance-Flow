...
export function TaskEditDialog({
  ...
  const handleTaskFormSubmit = async (
    ...
    // Show saving toast
    toast({
      title: T.saving || "Đang lưu...",
      description: T.savingDescription || "Dữ liệu đang được cập nhật lên Supabase...",
    });

    try {
      // Call the onSubmit handler (which updates Supabase)
      await onSubmit(values, quoteColumns, collaboratorQuoteColumns, taskId);

      // Show success notification
      toast({
        title: T.saveSuccess || "Đã lưu thành công!",
        description: T.saveSuccessDescription || "Dữ liệu đã được cập nhật.",
      });
      ...
      toast({
        variant: "destructive",
        title: T.saveFailed || "Lỗi khi lưu",
        description: T.saveFailedDescription || "Không thể lưu dữ liệu. Vui lòng thử lại.",
      });
    } finally {
      // Clear saving flags
      setIsSaving(false);
