...
import { analyzeProductivityAction } from '@/app/actions/ai-actions';
...
import { parseStructuredInsights } from '@/ai/utils/json-parser'; // Robust JSON parser
...
export function PredictionDashboard({ className = '' }: PredictionDashboardProps) {
  ...
  const { appData, T, workTime, handleDeleteTask, handleEditTask, setAppData } = useDashboard() as any;
  ...
  const prepareAIContext = () => {
    // Calculate periodDays from taskAnalytics range or from earliest data
    let periodDays = 30; // Default fallback
    // Use the state variable directly to ensure synchronization with the UI
    let rangeFrom = taskAnalyticsRange?.from;
    let rangeTo = taskAnalyticsRange?.to;
    let actualFromDate: Date | undefined = rangeFrom;
    let actualToDate: Date | undefined = rangeTo;

    if (rangeFrom && rangeTo) {
      const diffTime = Math.abs(rangeTo.getTime() - rangeFrom.getTime());
      periodDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)) || 1;
    } else {
      // "All time" - calculate from earliest task to today
    ...
    // Helper to check if a task falls within the date range
    const isInRange = (t: any) => {
      if (!rangeFrom || !rangeTo) return true;
      const taskDate = new Date(t.deadline || t.endDate || t.createdAt);
      return taskDate >= rangeFrom && taskDate <= rangeTo;
    };

    // Filter tasks by date range
    const filteredTasks = tasks.filter(isInRange);
    const completedInRange = filteredTasks.filter((t: any) => t.status === 'done');
    const activeInRange = filteredTasks.filter((t: any) => !['done', 'cancelled', 'archived'].includes(t.status));

    const context = {
      periodDays, // Add explicit period information for AI
      // Provide explicit ranges for clarity
      workTimeRange: productivityStats?.range ? {
        from: productivityStats.range.from?.toISOString() || '',
        to: productivityStats.range.to?.toISOString() || ''
      } : (productivityRange ? {
        from: productivityRange.from?.toISOString() || '',
        to: productivityRange.to?.toISOString() || ''
      } : null),
      ...
      // Filter tasks by selected date range for more relevant AI context
      recentCompleted: completedInRange
        .slice(-15)
...
INSTRUCTIONS:
1. Analyze the selected date range (${context.periodDays} days) only.
...
Return ONLY a valid JSON array (3-5) of StructuredInsight objects: [{"category":"...","severity":"...","insight":"...","suggestion":"..."}] with text in ${responseLanguage}. No markdown, no commentary.`;
      try {
        if (!finalModelName) throw new Error('Missing Model config');

        const response = await analyzeProductivityAction({
          apiKey: finalApiKey,
        ...
        if (response.success && response.insights) {
          // The server action already parsed the JSON
          ...
          // Save to appData (will be persisted to Supabase via dashboard context)
          if (typeof setAppData === 'function') {
            setAppData((prev: any) => ({ ...prev, aiProductivityAnalyses: updatedAnalyses }));
          }
        } else {
        ...
        toast({ variant: 'destructive', title: 'AI Call Failed', description: err.message });
      }
    } catch (error) {
