...
function DashboardContentInner({ searchParams }: { searchParams: ReadonlyURLSearchParams }) {
  ...
  const {
    ...
    handleRestoreProject, handlePermanentDeleteProject,
      ...
      <div className={cn("flex flex-col h-full gap-4", pathname.includes('/chat') || pathname.includes('/settings') || pathname.includes('/widgets') ? 'p-0' : 'p-4')}>
        ...
        <Card className="flex-1 flex flex-col min-h-0">
          <CardContent className="p-0 flex-1 overflow-y-auto">
            ...
            {(appData.projects || []).filter((p: Project) => p.deletedAt).length > 0 && (
              <div className="p-4 border-b">
                <h3 className="font-semibold mb-2 flex items-center gap-2">
                  <FolderOpen className="h-4 w-4" /> {T.deletedProjects || 'Deleted Projects'}
                </h3>
                <div className="space-y-2">
                  {(appData.projects || []).filter((p: Project) => p.deletedAt).map((project: Project) => (
                    <div key={project.id} className="flex items-center justify-between p-3 border rounded-lg bg-muted/30">
                      <div>
                        <p className="font-medium text-sm">{project.name}</p>
                        <p className="text-xs text-muted-foreground">
                          {T.deletedAt || 'Deleted'}: {new Date(project.deletedAt!).toLocaleString()}
                        </p>
                      </div>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                {T.deleteProjectPermanentlyDesc || 'This will permanently delete the project and all its data. This action cannot be undone.'}
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              <AlertDialogCancel>{T.cancel || 'Cancel'}</AlertDialogCancel>
                              <AlertDialogAction className={cn(buttonVariants({ variant: "destructive" }))} onClick={() => handlePermanentDeleteProject?.(project.id)}>
                                {T.delete || 'Delete'}
                              </AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
            ...
            <div className="p-4 bg-muted/10 border-b">
              <h3 className="font-semibold flex items-center gap-2">
                <CheckSquare className="h-4 w-4" /> {T.deletedTasks || 'Deleted Tasks'}
              </h3>
            </div>
        ...
        return (
          <div className="h-full w-full">
            <PertView
              ...
              categories={categories}
              collaborators={collaborators}
              projects={(appData.projects || []).filter((p: Project) => !p.deletedAt)}
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            <div className="flex-1 min-w-0">
              <FilterChipBar
                ...
                allCategories={categories} collaborators={collaborators} clients={clients} projects={(appData.projects || []).filter((p: Project) => !p.deletedAt)}
