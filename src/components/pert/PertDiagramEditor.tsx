...
interface PertDiagramEditorProps {
  ...
  // Project filtering
  projects?: any[]; // Keep any for now as Project type might be complex to import fully right now, but explicit any is better than implicit. Or import Project type.
...
const PertDiagramEditor: React.FC<PertDiagramEditorProps> = ({
    ...
    try {
      ...
      const projectFilteredTasks = effectiveSelectedProject && effectiveSelectedProject !== 'all'
        ? (effectiveSelectedProject === 'none'
          ? allTasks.filter((task: Task) => !task.projectId || task.projectId === 'none')
          : allTasks.filter((task: Task) => task.projectId === effectiveSelectedProject))
        : allTasks;
      ...
      const signature = JSON.stringify({
        ...
        projectTaskIds: projectFilteredTasks.map((t: Task) => t.id).sort(),
        taskProjectMappings: allTasks.map((t: any) => ({
          id: t.id,
      ...
      console.log('📝 Tasks signature updated:', {
        ...
        totalTasks: allTasks.length,
        projectTaskCount: projectFilteredTasks.length,
        tasksWithProject: allTasks.filter((t: any) => t.projectId).length,
  ...
  const convertTasksToPertDiagram = useCallback(() => {
    ...
    if (effectiveSelectedProject && effectiveSelectedProject !== 'all') {
      if (effectiveSelectedProject === 'none') {
        projectTasks = allDashboardTasks.filter((task: Task) => !task.projectId || task.projectId === 'none');
      } else {
        projectTasks = allDashboardTasks.filter((task: Task) => task.projectId === effectiveSelectedProject);
      }
    }
    ...
    projectTasks.forEach((task: Task, index: number) => {
      ...
      const taskNode: Node = {
        ...
        data: {
          ...
          onOpenPertDetails: (taskId: string) => {
            const taskToOpen = projectTasks.find((t: Task) => t.id === taskId);
            if (taskToOpen) {
    ...
    const finalTasks = projectTasks.filter((task: Task) => {
      // Tasks that are not dependencies of other tasks are final tasks
      return !projectTasks.some((otherTask: Task) =>
  ...
  useEffect(() => {
    ...
    if (dashboardTasks && effectiveSelectedProject && effectiveSelectedProject !== 'all') {
      const projectTasks = effectiveSelectedProject === 'none'
        ? dashboardTasks.filter((task: Task) => !task.projectId || task.projectId === 'none')
        : dashboardTasks.filter((task: Task) => task.projectId === effectiveSelectedProject);

      const projectTaskIds = projectTasks.map((t: Task) => t.id).sort();
      ...
      const isDifferent = projectTaskIds.length !== displayedTaskIds.length || projectTaskIds.some((id: string, i: number) => id !== displayedTaskIds[i]);
  ...
  const filteredTasks = taskSearch
    ? allTasks.filter((task: Task) => !task.deletedAt && task.name.toLowerCase().includes(taskSearch.toLowerCase()))
    : [];
        ...
        <div className="flex-1 w-full overflow-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {projects.map((p: any) => {
              ...
              const allTasks = dashboard?.appData?.tasks || tasks || [];
              const count = allTasks.filter((t: Task) => !t.deletedAt && t.projectId === p.id).length;
