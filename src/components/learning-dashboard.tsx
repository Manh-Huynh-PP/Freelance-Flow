    ...
    <div className="border rounded-lg p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <span className="font-medium text-sm">
            {(pattern.type || 'unknown').replace('_', ' ').toUpperCase()}
          </span>
        </div>
        <Badge variant="outline" className={getImpactColor(pattern.impact || 'neutral')}>
      ...
      <div className="flex justify-between items-center text-xs">
        <span className="text-gray-500">
          Strength: {((pattern.strength || 0) * 100).toFixed(0)}%
        </span>
        <span className="text-gray-500">
    ...
    <div className="border rounded-lg p-4 space-y-3">
      ...
      {prediction.personalizedFactors.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {prediction.personalizedFactors.map((factor: string, index: number) => (
        ...
        <div className="flex space-x-2 mt-1">
          {['too_high', 'accurate', 'too_low'].map((acc) => (
            <button
              ...
              className={`px-2 py-1 text-xs rounded ${accuracy === acc ? 'bg-blue-500 text-white' : 'bg-gray-200'
                }`}
            >
              {acc.replace('_', ' ')}
            </button>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              {patterns.length > 0 && patterns[0]?.type ? (
                <>Your strongest behavioral pattern is <strong>{(patterns[0].type || '').replace('_', ' ')}</strong> với {((patterns[0].strength || 0) * 100).toFixed(0)}% strength.</>
              ) : (
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {patterns.find(p => p.type === 'procrastination' && (p.strength || 0) > 0.6) && (
            <Alert>
...
function calculateLearningProgress(models: AdaptivePredictionModel[], feedback: FeedbackData[]) {
  ...
  const learningDuration = lastUpdated ? Date.now() - lastUpdated.getTime() : 0;
  ...
  return {
    ...
    learningDays: Math.ceil(learningDuration / (1000 * 60 * 60 * 24))
  };
}
