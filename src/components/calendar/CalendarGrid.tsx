...
export const CalendarGrid = (props: CalendarGridProps) => {
  ...
  const allEvents: AppEvent[] = events ?? dashboardContext?.events ?? [];
  ...
  const activeEvent = activeEventId ? allEvents.find((e: AppEvent) => `event-start-${e.id}` === activeEventId) : null;
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    if (!activeId || !overId || !overId.startsWith('date-')) {
      setDragOverDate(null);
      return;
    }
    setDragOverDate(overId.replace('date-', ''));
  };
