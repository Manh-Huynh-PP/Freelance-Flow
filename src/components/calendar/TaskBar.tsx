...
export const TaskBar: React.FC<TaskBarProps> = ({ task, dayStart, dayCount, scale, viewMode, statusColors, settings }) => {
  ...
  const language = (appSettings?.language || 'vi') as keyof typeof i18n;
  ...
  const mainStatus = appSettings?.statusSettings?.find((s: any) => s.id === task.status);
  let statusLabel = mainStatus?.label || task.status;
  if (task.subStatusId && mainStatus?.subStatuses) {
    const subStatus = mainStatus.subStatuses.find((ss: any) => ss.id === task.subStatusId);
    if (subStatus) {
