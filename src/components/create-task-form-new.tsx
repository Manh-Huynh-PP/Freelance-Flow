...
export const CreateTaskForm = forwardRef<CreateTaskFormRef, CreateTaskFormProps>((props, ref) => {
  ...
  const handleApplySuggestion = useCallback((items: SuggestQuoteOutput['suggestedItems']) => {
    ...
    const newItems = items.map((item: SuggestQuoteOutput['suggestedItems'][number], index: number) => ({
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-background min-h-0">
      <Form {...form}>
        <form ref={formRef} onSubmit={form.handleSubmit(handleFormSubmit)} className="flex flex-col h-full overflow-hidden">
