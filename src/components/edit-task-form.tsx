...
import {
  ...
  Settings, MoreHorizontal, Edit2, X, Loader2
} from "lucide-react";
...
import type { Task, Client, Collaborator, Category, Quote, QuoteSection, QuoteColumn, QuoteTemplate, AppSettings, Project } from "@/lib/types";
...
export function EditTaskForm({
          ...
          return {
            ...
            sections: cq.sections?.map(s => ({
              ...
              items: s.items?.map((item, itemIndex) => ({
                ...
                customFields: item.customFields || ((item.customFields as any)?.timeline ? item.customFields : {
  ...
  const form = useForm<TaskFormValues>({
    ...
    defaultValues: {
      ...
      projectId: taskToEdit?.projectId || undefined, // <--- Thêm default cho projectId
      dates: {
  ...
  const handleAddProject = useCallback(() => {
    ...
    const invalid = links.find(l => { try { new URL(l); return false; } catch { return true; } });
    if (invalid) {
  ...
  const handleApplySuggestion = useCallback((items: SuggestQuoteOutput['suggestedItems']) => {
    ...
    const newItems = items.map((item: { description: string; unitPrice: number }, index: number) => ({
  ...
  // DEBUG: Check if this component reaches the return statement
  console.warn('[EditTaskForm] 🔴 RENDER - formRef current:', formRef.current, 'settings:', settings?.language);

  return (
    <Form {...form}>
      <form ref={formRef} onSubmit={form.handleSubmit(onSubmit, onError)} className="flex flex-col h-full overflow-hidden">
        <div className="flex-1 overflow-y-auto px-1 space-y-4 md:space-y-6 pb-4">
          <FormField
        ...
        <div className="flex-none flex flex-col-reverse sm:flex-row justify-between gap-4 p-4 bg-background border-t z-10 mt-auto">
          ...
          <div className="flex justify-end gap-2">
            <Button type="button" variant="ghost" data-intent="close" onClick={() => setOpen(false)} disabled={isSaving}>
              {T.cancel}
            </Button>
            <Button type="submit" disabled={isSaving}>
              {isSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              {isSaving ? (T.saving || "Đang lưu...") : T.save}
            </Button>
          </div>
        </div>
