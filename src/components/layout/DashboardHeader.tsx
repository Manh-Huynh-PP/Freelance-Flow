...
import { PlusCircle, Download, HelpCircle } from "lucide-react";
import Link from 'next/link';
import { WorkTimeTracker } from '@/components/features/WorkTimeTracker';
import { PomodoroHeaderControl } from "@/components/header-pomodoro";
        ...
        <div className="flex items-center gap-5">
          <div className="flex items-center gap-3">
            <div className="hidden md:flex items-center gap-2 mr-2">
              <PomodoroHeaderControl settings={appData.appSettings} />
            </div>
            ...
            <div className="hidden md:inline-flex items-stretch rounded-2xl overflow-hidden bg-primary text-primary-foreground shadow-sm divide-x divide-primary-foreground/15 border-2">
              ...
              <Dialog open={isTaskFormOpen} onOpenChange={handleSetOpen}>
                ...
                <DialogContent
                  className={cn("max-h-[85vh] h-[85vh] flex flex-col overflow-hidden p-0 gap-0", { "sm:max-w-2xl md:max-w-5xl": "default" === taskFormSize, "sm:max-w-4xl md:max-w-7xl": "large" === taskFormSize, "w-screen h-screen max-w-none max-h-none rounded-none border-0": "fullscreen" === taskFormSize })}
                ...
                >
                  <DialogHeader className="text-center pt-6 px-6 pb-2 flex-none sm:px-10"><DialogTitle>{T.createTask}</DialogTitle><DialogDescription>{T.createTaskDesc}</DialogDescription></DialogHeader>
