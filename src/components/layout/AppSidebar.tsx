              ...
              {isManageOpen && (
                <SidebarGroupContent id="sidebar-group-manage">
                  <SidebarMenu>
                    ...
                    <SidebarMenuItem>
                      <SidebarMenuButton onClick={() => setIsFixedCostManagerOpen(true)}>
                        <DollarSign />{T.fixedCosts || 'Fixed Costs'}
                      </SidebarMenuButton>
                      <FixedCostsCard
                        dateRange={{}}
                        hideSummary
                        ...
                        open={isFixedCostManagerOpen}
                        onOpenChange={setIsFixedCostManagerOpen}
                      />
                    </SidebarMenuItem>
                    ...
                    <SidebarMenuItem>
                      <Dialog open={isTemplateManagerOpen} onOpenChange={setIsTemplateManagerOpen}>
                        ...
                        <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>{T.quoteTemplateManagement}</DialogTitle></DialogHeader><QuoteTemplateManager templates={appData.quoteTemplates} onAddTemplate={(values, columns) => setAppData((prev: any) => ({ ...prev, quoteTemplates: [...prev.quoteTemplates, { id: `template-${Date.now()}`, name: values.name, sections: values.sections, columns }] }))} onEditTemplate={(template) => setAppData((prev: any) => ({ ...prev, quoteTemplates: prev.quoteTemplates.map((t: any) => t.id === template.id ? template : t) }))} onDeleteTemplate={(id) => setAppData((prev: any) => ({ ...prev, quoteTemplates: prev.quoteTemplates.filter((t: any) => t.id !== id) }))} language={appData.appSettings.language} settings={appData.appSettings} /></DialogContent>
                      </Dialog>
                    </SidebarMenuItem>
                  </SidebarMenu>
                </SidebarGroupContent>
                      ...
                      <SidebarMenuItem>
                        <SidebarMenuButton asChild>
                          <div className="flex items-center gap-2 px-2 py-1 text-sm">
                            ...
                            <span className="truncate">{session.user?.email || (session.user as any)?.name || 'User'}</span>
                          </div>
                        </SidebarMenuButton>
                      </SidebarMenuItem>
                      <SidebarMenuItem>
